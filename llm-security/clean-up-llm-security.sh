...
DEVELOPER_ID=$(apigeecli developers get --email llm-security-developer@acme.com --org "$PROJECT_ID" --token "$TOKEN" --disable-check | jq .'developerId' -r)
apigeecli apps delete --id "$DEVELOPER_ID" --name llm-security-app --org "$PROJECT_ID" --token "$TOKEN"

echo "Deleting Developer"
apigeecli developers delete --email llm-security-developer@acme.com --org "$PROJECT_ID" --token "$TOKEN"
